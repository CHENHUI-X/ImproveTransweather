...
from utils.arch_util import to_2tuple, trunc_normal_
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            ...
            self.conv = nn.Sequential(
                nn.Conv2d(dim, dim // 4, 3, 1, 1, groups=dim // 4), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(dim // 4, dim // 4, 1, 1, 0, groups=dim // 4), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(dim // 4, dim, 3, 1, 1, groups=dim // 4))
    ...
    def forward(self, x, x_size):
        return self.after_PSA_Group_Residual(
            self.after_PSA_Group_conv(
                self.patch_embed(
                    self.conv(self.patch_unembed(self.before_PSA_Group_conv(self.residual_group(x, x_size)), x_size))
                )
            ) + x
        )
...
class SRFormer(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv_after_body = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1, groups=embed_dim)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv_after_body = nn.Sequential(
                nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1, groups=embed_dim // 4),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1, 0, groups=embed_dim // 4),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(embed_dim // 4, embed_dim, 3, 1, 1, groups=embed_dim // 4))
